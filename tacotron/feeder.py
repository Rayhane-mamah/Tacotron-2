...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        with tf.device('/cpu:0'):
            ...
            tf.placeholder(tf.int32, shape=(hparams.num_gpus, None), name='split_infos'),
            ]
    ...
    def _prepare_batch(self, batches, outputs_per_step):
        assert 0 == len(batches) % self._hparams.num_gpus
        size_per_device = int(len(batches) / self._hparams.num_gpus)
        np.random.shuffle(batches)
