...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, split_infos=None, linear_targets=None, targets_lengths=None, gta=False,
        ...
        with tf.device('/cpu:0') :
            ...
            tower_targets_lengths = tf.split(targets_lengths, num_or_size_splits=hp.num_gpus, axis=0) if targets_lengths is not None else targets_lengths

            p_inputs = tf.py_func(split_func, [inputs, split_infos[:, 0]], lout_int)
            p_mel_targets = tf.py_func(split_func, [mel_targets, split_infos[:,1]], lout_float)
        ...
        gpus = ["/gpu:{}".format(i) for i in range(hp.gpu_start_idx, hp.gpu_start_idx+hp.num_gpus)]
        for i in range(hp.num_gpus):
            with tf.device(tf.train.replica_device_setter(ps_tasks=1,ps_device="/cpu:0",worker_device=gpus[i])):
                with tf.variable_scope('inference') as scope:
                    ...
                    embedded_inputs = tf.nn.embedding_lookup(embedding_table, tower_inputs[i])
                    ...
                    attention_mechanism = LocationSensitiveAttention(hp.attention_dim, encoder_outputs, hparams=hp,
                        mask_encoder=hp.mask_encoder, memory_sequence_length=tower_input_lengths[i], smoothing=hp.smoothing,
                    ...
                    if is_training or is_evaluating or gta:
                        self.helper = TacoTrainingHelper(batch_size, tower_mel_targets[i], tower_stop_token_targets[i], hp, gta, is_evaluating, global_step)
                    else:
        ...
        for i in range(hp.num_gpus+hp.gpu_start_idx):
            ...
            if post_condition:
                log('  linear out:               {}'.format(self.towerlinear_outputs[i].shape))
    ...
    def add_loss(self):
        ...
        #gpus = ["/gpu:{}".format(i) for i in range(hp.num_gpus)]
        gpus = ["/gpu:{}".format(i) for i in range(hp.gpu_start_idx, hp.gpu_start_idx+hp.num_gpus)]
        ...
        self.before_loss = total_before_loss / hp.num_gpus
        self.after_loss = total_after_loss / hp.num_gpus
        self.stop_token_loss = total_stop_token_loss / hp.num_gpus
        self.regularization_loss = total_regularization_loss / hp.num_gpus
        self.linear_loss = total_linear_loss / hp.num_gpus
        self.loss = total_loss / hp.num_gpus
